...
const showProducts = (products) => {
   if (!products[0]) {setInnerText('total_products', 0); document.getElementById("all-products").innerHTML = "<h1>Error!!!</h1>"; return;} 
   ...
   for (const product of allProducts) {
    ...
    <img class="product-image" src="${image}"></img>
      ...
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="btn btn-success border-0 w-100 rounded-0 bg-main py-2">Add to cart</button>
      </div>`;
...
const showProductDetailsInModal = (product_details) => {
   ...
   document.getElementById("mod-img").src = product_details.image;
// setInnerText('', );
...
const updatePrice = (id, value) => {
   ...
   document.getElementById("total").innerText = Math.round(total) + 20;
};
...
document.getElementById("search-btn").addEventListener("click", function () {
   ...
   const searchedProduct = arr[0].find((p) =>
     p.category.startsWith(inputField)); arR.push(searchedProduct);
